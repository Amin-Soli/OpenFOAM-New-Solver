...
{
    defineTypeNameAndDebug(newForces, 0);

    addToRunTimeSelectionTable(functionObject, newForces, dictionary);
}
}
...
Foam::wordList Foam::functionObjects::newForces::createFileNames
(
...
void Foam::functionObjects::newForces::writeFileHeader(const label i)
{
    if (i == 0)
    {
        // force data
...
void Foam::functionObjects::newForces::initialise()
{
    if (initialised_)
    {
        return;
...
Foam::functionObjects::newForces::devRhoReff() const
{
    ...
    {           
        ...
        const volScalarField& alpha1 = obr_.lookupObject<volScalarField>("alpha.water");
        
        const volVectorField& U = obr_.lookupObject<volVectorField>(UName_);
        
	    dimensionedScalar nu1(transportProperties.subDict("water").lookup("nu"));
        dimensionedScalar nu2(transportProperties.subDict("vapour").lookup("nu"));
        dimensionedScalar rho1(transportProperties.subDict("water").lookup("rho"));
        dimensionedScalar rho2(transportProperties.subDict("vapour").lookup("rho"));
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::newForces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::newForces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::newForces::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::newForces::applyBins
(
...
void Foam::functionObjects::newForces::writeForces()
{
...
void Foam::functionObjects::newForces::writeBins()
{
    if (nBin_ == 1)
    {
        return;
...
Foam::functionObjects::newForces::newForces
(
    const word& name,
    const Time& runTime,
    const dictionary& dict
...
bool Foam::functionObjects::newForces::read(const dictionary& dict)
{
...
void Foam::functionObjects::newForces::calcForcesMoment()
{
    initialise();
...
Foam::vector Foam::functionObjects::newForces::forceEff() const
{
    return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


Foam::vector Foam::functionObjects::newForces::momentEff() const
{
    return sum(moment_[0]) + sum(moment_[1]) + sum(moment_[2]);
}


bool Foam::functionObjects::newForces::execute()
{
    return true;
}


bool Foam::functionObjects::newForces::write()
{
    calcForcesMoment();
